...
class LLMModule:
    ...
    def check_correct_letter(self, letter: str):
        ...
        Перед тобой текст, предположительно являющийся письмом советского солдата периода 1941–1945 годов. Твоя задача — определить, подходит ли он по смыслу, стилю и общей достоверности для дальнейшей обработки.

        Пропускай письма, если они:
        – написаны простым, живым языком с бытовыми деталями, описанием службы, мыслями о родных, повседневными трудностями;
        – содержат эмоциональные фразы, разговорную речь, орфографические или стилистические ошибки;
        – выглядят реалистично, даже если краткие, небрежные или неполные.

        Не пропускай письма, если они:
        – содержат современный сленг, интернет-мемы, шутки, провокации, бессмысленный набор слов;
        – являются рекламой, спамом или сгенерированным шумом;
        – полностью нерелевантны эпохе и контексту фронтового письма.
