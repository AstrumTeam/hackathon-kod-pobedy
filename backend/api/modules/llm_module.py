...
class LLMModule:
    ...
    def check_correct_letter(self, letter: str):
        ...
        Перед тобой текст, предположительно являющийся письмом советского солдата периода 1941–1945 годов. Твоя задача — определить, соответствует ли он стилю, содержанию и лексике подлинных военных писем того времени.

        Критерии:

        – Если письмо написано в характерной для 1940-х годов манере (простая речь, бытовые детали, обращения к родным, описание службы, тоска по дому и т.п.) — считай его подлинным.  
        – Если текст содержит современный сленг, рекламные фразы, нерелевантную информацию или выглядит как бессмысленный набор слов — считай его фальшивым.  
        – Если не уверен, выбирай в пользу подлинности.

        Ответ строго в формате JSON:
        - {"correct": true} — если письмо похоже на настоящее письмо советского солдата времён 1941–1945 годов.
        - {"correct": false} — если письмо похоже на спам, фальшивку или бессмысленный текст.
        ...
        Перед тобой текст, якобы являющийся письмом советского солдата периода 1941–1945 годов. Твоя задача — определить, подходит ли этот текст для дальнейшей обработки: генерации сцены или аудио.

        Фильтруй жёстко. Пропускай только те письма, которые:

        – содержат реалистичное и последовательное повествование,
        – написаны в стиле и лексике 1940-х годов (простая, бытовая, личная речь),
        – не содержат абсурда, насмешек, искажений, современных фраз, интернет-сленга, мата, мемов и троллинга,
        – не являются рекламой, спамом, пустыми сообщениями, генеративным шумом или бессмысленным набором слов.

        Если письмо вызывает сомнение, но выглядит реалистично — пропусти.  
        Если оно выглядит как выдумка, шутка, провокация или просто нерелевантный текст — отклони.

        Ответ строго в формате JSON:
        - {"correct": true} — если письмо подходит.
        - {"correct": false} — если письмо нужно отфильтровать.
